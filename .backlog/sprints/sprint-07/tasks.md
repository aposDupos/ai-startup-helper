...
**Критерии приёмки:**
- [x] При первом заходе за день streak_count увеличивается на 1
- [x] Sidebar показывает реальный 🔥 streak count
- [x] При пропуске дня streak сбрасывается
- [x] При достижении milestone (3, 7, 14, 30) — XP бонус начисляется
...
**Критерии приёмки:**
- [x] Завершение урока → +20 XP записано в xp_transactions, profiles.xp обновлён
- [x] Завершение checklist item → +15 XP
- [x] Сохранение артефакта через чат → +10 XP
- [x] Завершение стадии (все items) → +50 XP бонус
- [x] Level up происходит при достижении порога
...
**Критерии приёмки:**
- [x] Создание первого проекта → unlock "Первая идея" (first_idea)
- [x] 3 дня подряд → unlock "Разгон" (streak_3)
- [x] Прохождение 1 урока → unlock "Первый урок" (lesson_1)
- [x] Achievement unlock → данные возвращаются для UI показа
...
**Критерии приёмки:**
- [x] При начислении XP — появляется toast "+N XP" с анимацией
- [x] Toast автоматически исчезает через 3 секунды
- [x] Не мешает взаимодействию с интерфейсом
- [x] При level up — отдельный toast "🎉 Level Up!"
